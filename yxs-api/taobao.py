...
def urlopen(*args,**kwargs):
    ...
    req_start_time = time.strftime('%Y-%m-%d %H:%M:%S',time.localtime())
    ## 直接copy淘宝url
    print('当前时间：{0}'.format(req_start_time))
    try:
...
def __excel_writer(data):
    ## 写入excel文件
    excel_path = os.path.join(os.getcwd(), 'taobao.xls')
    excel = ExcelUntil(excel_path)
    ## 如果有文件则直接打开，没有则新建一个
    if not os.path.exists(excel_path):
        excel.create_Excel((0, 0, 'title'), (0, 1, 'raw_title'), (0, 2, 'item_loc'), (0, 3, 'view_sales'), (0, 4, 'nick'), (0, 5, 'delivery'),
                           (0, 6, 'view_price'), (0, 7, 'view_fee'), (0, 8, 'detail_url'),(0,9,'shoucang'),(0,10,'pingjia'))
